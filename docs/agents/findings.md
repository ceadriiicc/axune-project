...
| C2 | `--sandbox read-only` blocks **all process creation** on Windows, so Codex cannot even read a file. PowerShell and `cmd.exe` are both refused with `CreateProcess ... rejected: blocked by policy`. Unaffected by `--ignore-user-config` or `--ignore-rules`. | **verified** — four live runs, 2026-09-08 |
...
| Claim | Position |
|---|---|
| "Use Codex sandboxing as the enforcement boundary, run with `-C` against the isolated worktree." — Codex, 2026-09-09 | **Refuted on Windows.** Sound in principle; C2 and C3 make it impossible here. Recorded because it is probably right on macOS and Linux, where the sandbox is Seatbelt/Landlock. Not adopted. |
