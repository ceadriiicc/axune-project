...
| C29 | **`codex mcp list` is a free oracle for the live tool surface** - no model run, no usage. Against the user's main home it lists `cua_repl` and `node_repl` as `enabled`. It still lists both when passed `-c 'plugins."computer-use@openai-bundled".enabled=false'` (and the same for unified-computer-use and browser), and when passed `-c 'mcp_servers={}'`. So **no override removes the computer-use MCP servers from the user's own profile.** Use this command before spending a run on any containment question. | **verified** - 2026-09-09 |
| C30 | **Q7 answered: NO. Neither configuration is both safe and functional.** A dedicated `CODEX_HOME` *does* remove the MCP/computer-use surface - a live run reached no `cua_repl` at all - but it fails twice over: `.sandbox` is never provisioned in a fresh home, so every command is refused and nothing can be read (the C2 symptom, now shown to be about provisioning state rather than the flag); and **`web_search` is a built-in tool, not a plugin**, so it ran anyway with a real query. The user's main home keeps the sandbox but cannot have the tool surface removed (C29). | **verified by breach attempt** - 2026-09-09 |
| C31 | Unexplored levers, for whoever returns to this: `allowed_web_search_modes`, `external_web_access`, `indexed_web_access` and `sandbox_network_access` appear as config vocabulary in the binary and may disable built-in web search. Sandbox provisioning in a fresh home might also be reachable by copying `.sandbox`, `.sandbox-bin` and `.sandbox-secrets` from the main home - but that last one holds DPAPI-protected sandbox-user credentials, so it is a fragile hack rather than a design. | **unverified** - strings only |

| C32 | **Built-in `web_search` cannot be disabled by configuration. Codex is ruled out for Axune.** Codex verified `external_web_access=false` fails, and documented that `--search` is an enable-only switch with no corresponding disable flag. Claude then set `allowed_web_search_modes=[]`, `indexed_web_access=false`, `external_web_access=false` and `sandbox_network_access=false` **together**, under `--sandbox read-only --ignore-rules`, and a run asked for the weather called `web_search` three times and returned live forecast data. There is no free oracle for this tool - `codex debug prompt-input` does not expose the model's tool manifest - so it can only be tested by attempting it. | **verified by breach attempt** - 2026-09-09, by both agents independently |
